...
export class OGCAPI {
  ...
  setup() {
    ...
    if (!root.isProviderConformed()) {
      throw new Error('Provider does not conform to required core classes for RootHandler');
    }
    root.setupRoutes(this.router);


    const collectionHandler = new CollectionHandler(this.provider, this.options);
